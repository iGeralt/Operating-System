  ...
  *check = 0;
  ...
  if(p == 0)
  {
    ...
    else if(c1 == 0 && c2 > 0)
    {
      //third part
      int *c;
      c = min_element(arr + n/2,arr + (n/4)*3);
      *i3 = *c;
      exit(EXIT_SUCCESS);
    }
    else if(c1 == 0 && c2 == 0)
  ...
  else
  {
    ...
    while(1) // fourth child sometimes runs before parent so this loop has been used to eradicate that problem
    {
      if(*check == 1)
      {
        ...
        munmap(i3,sizeof *i3);
        munmap(i4,sizeof *i4);
        i5 = min_element(new_arr + 0,new_arr + 4);
        cout<<*i5<<endl;
        break;
      }
    }
